...
export default function Nav() {
  ...
  return <>
    <div className={collapsed ? 'fixed top-4 left-1/2 -translate-x-1/2 z-[var(--z-nav)] w-[calc(100%-2rem)] md:w-auto max-w-[380px]' : 'fixed top-0 inset-x-0 z-[var(--z-nav)] w-full'}>
      <motion.nav layout className={collapsed ? 'w-full flex items-center justify-between gap-4 pl-4 pr-1.5 py-1.5 rounded-[var(--radius-pill)] backdrop-blur-xl bg-[var(--bg-surface)]/80 border border-[color:var(--border-default)]' : 'w-full flex items-center justify-between px-6 md:px-12 py-5 bg-transparent'} transition={{ duration: .4, ease: [.16, 1, .3, 1] }}>
